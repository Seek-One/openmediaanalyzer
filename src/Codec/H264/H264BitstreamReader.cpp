...
void H264BitstreamReader::readNALHeader(H264NALHeader& h264NALHeader)
{
	// Ref sec. 7.3.1
	h264NALHeader.forbidden_zero_bit = readBits(1);
	h264NALHeader.nal_ref_idc = readBits(2);
	h264NALHeader.nal_unit_type = (H264NALUnitType::Type)readBits(5);
}
...
void H264BitstreamReader::readPPS(H264PPS& h264PPS)
{
	...
	if (h264PPS.num_slice_groups_minus1 > 0) {
		...
		if (h264PPS.slice_group_map_type == 0) {
			for (uint32_t iGroup = 0; iGroup <= h264PPS.num_slice_groups_minus1; iGroup++) {
...
{
	h264Slice.IdrPicFlag = (h264Slice.getNalUnitType() == H264NALUnitType::IDRFrame);
	...
	if (h264Slice.getNALHeader()->nal_ref_idc != 0)
	{
...
{
	if (h264Slice.getNALHeader()->nal_unit_type == H264NALUnitType::IDRFrame)
	{
	...
	if(activeSPS.pic_struct_present_flag){
		...
		for(int i = 0;i < NumClockTS;++i){
			...
			if(h264SEImsg->clock_timestamp_flag[i]){
				...
				if(h264SEImsg->full_timestamp_flag[i]){
					...
					if(h264SEImsg->seconds_flag[i]){
						...
						if(h264SEImsg->minutes_flag[i]){
							...
							if(h264SEImsg->hours_flag[i]) h264SEImsg->hours_value[i] = readBits(5);
						}
					}
				}
