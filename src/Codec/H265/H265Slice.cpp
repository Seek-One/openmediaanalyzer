...
void H265Slice::validate(){
	H265NAL::validate();
	if(slice_pic_parameter_set_id > 63) minorErrors.push_back((std::ostringstream() << "[Slice] slice_pic_parameter_set_id value (" << (int)slice_pic_parameter_set_id << ") not in valid range (0..63)").str());
	H265PPS* pPps = getPPS();
	if(!pPps){
		majorErrors.push_back((std::ostringstream() << "[Slice] reference to unknown PPS (" << (int)slice_pic_parameter_set_id << ")").str());
		return;
	}
	H265SPS* pSps = getSPS();
	if(!pSps){
		majorErrors.push_back((std::ostringstream() << "[Slice] reference to unknown SPS (" << (int)pPps->pps_seq_parameter_set_id << ")").str());
		return;
	}
	H265VPS* pVps = getVPS();
	if(!pVps) majorErrors.push_back((std::ostringstream() << "[Slice] reference to unknown VPS (" << (int)pSps->sps_video_parameter_set_id << ")").str());
	if(pPps->TemporalId > TemporalId) minorErrors.push_back((std::ostringstream() << "[Slice] referenced PPS has a greater TemporalId value").str());
	if(pPps->nuh_layer_id > nuh_layer_id) minorErrors.push_back((std::ostringstream() << "[Slice] referenced PPS has a greater nuh_layer_id value").str());
	if(pSps->nuh_layer_id > nuh_layer_id) minorErrors.push_back((std::ostringstream() << "[Slice] referenced SPS has a greater nuh_layer_id value").str());
	if(slice_segment_address > pSps->PicSizeInCtbsY-1) minorErrors.push_back((std::ostringstream() << "[Slice] slice_segment_address value (" << slice_segment_address << ") not in valid range (0.." << pSps->PicSizeInCtbsY-1 << ")").str());
	if(!pPps->dependent_slice_segments_enabled_flag){
		if(slice_type > 2) minorErrors.push_back((std::ostringstream() << "[Slice] slice_type value (" << (int)slice_type << ") not in valid range (0..2)").str());
		if(nal_unit_type >= UnitType_BLA_W_LP && nal_unit_type <= UnitType_IRAP_VCL23 &&
			...
			slice_type != 2) minorErrors.push_back((std::ostringstream() << "[Slice] slice_type value of an IRAP picture not equal to 2").str());
		if(pSps->sps_max_dec_pic_buffering_minus1[TemporalId] == 0 && nuh_layer_id == 0 &&
			...
			minorErrors.push_back((std::ostringstream() << "[Slice] slice_type value not equal to 2").str());
		}
		if(colour_plane_id > 2) minorErrors.push_back((std::ostringstream() << "[Slice] colour_plane_id value (" << (int)colour_plane_id << ") not in valid range (0..2)").str());
	}
	if(slice_pic_order_cnt_lsb > pSps->MaxPicOrderCntLsb-1) minorErrors.push_back((std::ostringstream() << "[Slice] slice_pic_order_cnt_lsb value (" << slice_pic_order_cnt_lsb << ") not in valid range (0.." << pSps->MaxPicOrderCntLsb-1 << ")").str());
	if(short_term_ref_pic_set_idx > pSps->num_short_term_ref_pic_sets-1) minorErrors.push_back((std::ostringstream() << "[Slice] short_term_ref_pic_set_idx value (" << (int)short_term_ref_pic_set_idx << ") not in valid range (0.." << pSps->num_short_term_ref_pic_sets-1 << ")").str());
	if(num_long_term_sps > pSps->num_long_term_ref_pics_sps) minorErrors.push_back((std::ostringstream() << "[Slice] num_long_term_sps value (" << num_long_term_sps << ") not in valid range (0.." << pSps->num_long_term_ref_pics_sps << ")").str());
	...
	if(nuh_layer_id == 0 && num_long_term_pics > num_long_term_pics_limit){
		minorErrors.push_back((std::ostringstream() << "[Slice] num_long_term_pics value (" << num_long_term_pics << ") not in valid range (0.." << num_long_term_pics_limit << ")").str());
	}
	for(int i = 0;i < lt_idx_sps.size();++i){
		if(lt_idx_sps[i] > pSps->num_long_term_ref_pics_sps-1) minorErrors.push_back((std::ostringstream() << "[Slice] lt_idx_sps[" << i << "] value (" << lt_idx_sps[i] << ") not in valid range (0.." << pSps->num_long_term_ref_pics_sps-1 << ")").str());
	}
	...
	for(int i = 0;i < delta_poc_msb_cycle_lt.size();++i){
		if(delta_poc_msb_cycle_lt[i] > delta_poc_msb_cycle_lt_limit) minorErrors.push_back((std::ostringstream() << "[Slice] delta_poc_msb_cycle_lt[" << i << "] value (" << delta_poc_msb_cycle_lt[i] << ") not in valid range (0.." << delta_poc_msb_cycle_lt_limit << ")").str());
	}
	if(slice_type == SliceType_P || slice_type == SliceType_B){
		if(num_ref_idx_active_override_flag){
			if(num_ref_idx_l0_active_minus1 > 14) minorErrors.push_back((std::ostringstream() << "[Slice] num_ref_idx_l0_active_minus1 value (" << num_ref_idx_l0_active_minus1 << ") not in valid range (0..14)").str());
			if(num_ref_idx_l1_active_minus1 > 14) minorErrors.push_back((std::ostringstream() << "[Slice] num_ref_idx_l1_active_minus1 value (" << num_ref_idx_l1_active_minus1 << ") not in valid range (0..14)").str());
		}
		if(collocated_from_l0_flag && (slice_type == SliceType_P || slice_type == SliceType_B)  && collocated_ref_idx > num_ref_idx_l0_active_minus1){
			minorErrors.push_back((std::ostringstream() << "[Slice] collocated_ref_idx value (" << collocated_ref_idx << ") not in valid range (0.." << num_ref_idx_l0_active_minus1 << ")").str());
		}
		// if(slice_temporal_mvp_enabled_flag) ...
		// pred_weight_table
		if(five_minus_max_num_merge_cand > 4) minorErrors.push_back((std::ostringstream() << "[Slice] five_minus_max_num_merge_cand value (" << five_minus_max_num_merge_cand << ") not in valid range (0..4)").str());
	}
	if(SliceQpY < -pSps->QpBdOffsetY || SliceQpY > 51) minorErrors.push_back((std::ostringstream() << "[Slice] SliceQpY value (" << SliceQpY << ") not in valid range (" << -pSps->QpBdOffsetY << "..51)").str());
	if(slice_cb_qp_offset < -12 || slice_cb_qp_offset > 12) minorErrors.push_back((std::ostringstream() << "[Slice] slice_cb_qp_offset value (" << slice_cb_qp_offset << ") not in valid range (-12..12)").str());
	if(slice_cr_qp_offset < -12 || slice_cr_qp_offset > 12) minorErrors.push_back((std::ostringstream() << "[Slice] slice_cr_qp_offset value (" << slice_cr_qp_offset << ") not in valid range (-12..12)").str());
	if(slice_beta_offset_div2 < -6 || slice_beta_offset_div2 > 6) minorErrors.push_back((std::ostringstream() << "[Slice] slice_beta_offset_div2 value (" << slice_beta_offset_div2 << ") not in valid range (-6..6)").str());
	if(slice_tc_offset_div2 < -6 || slice_tc_offset_div2 > 6) minorErrors.push_back((std::ostringstream() << "[Slice] slice_tc_offset_div2 value (" << slice_tc_offset_div2 << ") not in valid range (-6..6)").str());
	...
	if(num_entry_point_offsets > num_entry_point_offsets_limit) minorErrors.push_back((std::ostringstream() << "[Slice] num_entry_point_offsets value (" << num_entry_point_offsets << ") not in valid range (0.." << num_entry_point_offsets_limit << ")").str()); 
	if(offset_len_minus1 > 31) minorErrors.push_back((std::ostringstream() << "[Slice] offset_len_minus1 value (" << offset_len_minus1 << ") not in valid range (0..31)").str()); 
}
