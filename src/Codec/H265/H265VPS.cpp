...
H265VPS::H265VPS():
	H265VPS(0, H265NALUnitType::Unspecified, 0, 0, 0, nullptr)
{}

H265VPS::H265VPS(uint8_t forbidden_zero_bit, H265NALUnitType::Type nal_unit_type, uint8_t nuh_layer_id, uint8_t nuh_temporal_id_plus1, uint32_t nal_size, const uint8_t* nal_data):
...
void H265VPS::validate()
{
	...
	if(vps_max_layers_minus1 > 62){
		errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_max_layers_minus1 value (%ld) not in valid range (0..62)", vps_max_layers_minus1));
	}
	if(vps_max_sub_layers_minus1 > 6){
		errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_max_sub_layers_minus1 value (%ld) not in valid range (0..6)", vps_max_sub_layers_minus1));
	}
	...
	for (uint8_t i = (vps_sub_layer_ordering_info_present_flag ? 0 : vps_max_sub_layers_minus1); i <= vps_max_sub_layers_minus1; ++i) {
		if(!vps_base_layer_internal_flag){
			if(vps_max_dec_pic_buffering_minus1[i] != 0){
				errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_max_dec_pic_buffering_minus1[%d] value (%ld) not 0 (no base layer)", i, vps_max_dec_pic_buffering_minus1[i]));
			}
			if(vps_max_num_reorder_pics[i] != 0){
				errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_max_num_reorder_pics[%d] value (%ld) not 0 (no base layer)", i, vps_max_num_reorder_pics[i]));
			}
			if(vps_max_latency_increase_plus1[i] != 0){
				errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_max_latency_increase_plus1[%d] value (%ld) not 0 (no base layer)", i, vps_max_latency_increase_plus1[i]));
			}
		}
	}
	if(vps_max_layer_id > 62){
		errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_max_layer_id value (%ld) not in valid range (0..62)", vps_max_layer_id));
	}
	if(vps_num_layer_sets_minus1 > 1023){
		errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_num_layer_sets_minus1 value (%ld) not in valid range (0..1023)", vps_num_layer_sets_minus1));
	}
	if (vps_timing_info_present_flag) {
		...
		if (vps_poc_proportional_to_timing_flag) {
			if(vps_num_ticks_poc_diff_one_minus1 == UINT32_MAX){
				errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_num_ticks_poc_diff_one_minus1 value (%ld) not in valid range (0..4294967294)", vps_num_ticks_poc_diff_one_minus1));
			}
		}
		if(vps_num_hrd_parameters > vps_num_layer_sets_minus1+1u){
			errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] vps_num_hrd_parameters value (%ld) not in valid range (0..{})", vps_num_hrd_parameters, vps_num_layer_sets_minus1+1));
		}
		for (uint32_t i = 0; i < vps_num_hrd_parameters; ++i) {
			if((!vps_base_layer_internal_flag && hrd_layer_set_idx[i] == 0) || hrd_layer_set_idx[i] > vps_num_layer_sets_minus1){
				errors.add(H26XError::Minor, H26XUtils::formatString("[VPS] hrd_layer_set_idx[%d] value (%ld) not in valid range ({}..{})", i, hrd_layer_set_idx[i], (vps_base_layer_internal_flag ? 0 : 1), vps_num_layer_sets_minus1));
			}
